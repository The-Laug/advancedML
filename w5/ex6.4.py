...
def plot_optimization_results(start_tensor, end_tensor, optimized_tensor, metric_fn=None, gridsize=100):
        """
        Plots the original straight line, the optimized curve, and the
        energy landscape of the metric space.

        Parameters
        - start_tensor, end_tensor: torch tensors shape (2,)
        - optimized_tensor: tensor shape (N,2)
        - metric_fn: callable (grid_x, grid_y) -> grid_z giving a scalar cost
            for each grid point. If None, uses the default scalar metric
            `1 + x^2 + y^2` used previously.
        - gridsize: resolution of the background grid for contour plotting.
        """
    ...
    # Calculate scalar cost grid using provided metric_fn or default
    if metric_fn is None:
        grid_z = 1.0 + grid_x**2 + grid_y**2
    else:
        # metric_fn should accept numpy arrays of same shape as grid_x/grid_y
        grid_z = metric_fn(grid_x, grid_y)
    ...
    ax = plt.gca()
    ax.set_title('Curve Optimization in a Custom Metric Space')
    ax.set_xlabel('X')
    ax.set_ylabel('Y')
    ax.legend()
    ax.grid(True, linestyle=':', alpha=0.7)
    ax.set_aspect('equal')  # Ensures x and y axes have the same scale

    # Set limits after plotting so they are not overridden by autoscale
